...
lisa_classify <- function(x,neighbours){
  ...
  d <- d %>% mutate(LISA_Cluster=case_when(LISA_p<0.05 & Indicator>=0 & Indicator_Lagged>=0~"High-High"))
...
```{r}
LGAs_by_Violence %>% pull(Num_Violence) %>% moran.test(neighbours_weights)
```

# Moran's Scatterplot

```{r}
moran.plot(LGAs_by_Violence$Num_Violence,neightbours_weights)
```
